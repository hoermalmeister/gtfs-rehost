...
def manage_backups(current_zip):
    ...
    # 1. RETENCE: Smazání záloh DÚK starších než 120 dní (cca 4 měsíce)
    for b in glob.glob(os.path.join(backup_dir, 'duk_*.zip')):
        date_str = os.path.basename(b).replace('duk_', '').replace('.zip', '')
        try:
    ...
    backups = sorted(glob.glob(os.path.join(backup_dir, 'duk_*.zip')))
    needs_backup = True
    
    if backups:
        last_date_str = os.path.basename(backups[-1]).replace('duk_', '').replace('.zip', '')
        try:
...
def filter_gtfs():
    print("Spouštím filtraci dat DÚK...")
    ...
    stops = stops[stops['zone_id'].fillna('').str.contains('U')]
